CREATE OR REPLACE VIEW regression_base_hourly AS
WITH machines AS (
...
SELECT
    ...
    COALESCE(sth.planned_time_sec, 0) AS planned_time_sec,
    COALESCE(sth.availability_loss_sec, 0) AS availability_loss_sec,

    COALESCE(csh.ideal_cycle_time_total_sec, 0) AS ideal_cycle_time_total_sec,
    COALESCE(csh.actual_cycle_time_total_sec, 0) AS actual_cycle_time_total_sec,

    COALESCE(csh.total_parts, 0) AS total_parts,
    COALESCE(csh.good_parts, 0) AS good_parts,
    COALESCE(csh.scrap_parts, 0) AS scrap_parts,

    COALESCE(seh.energy_total_kwh, 0) AS energy_total_kwh,
    COALESCE(seh.energy_productive_kwh, 0) AS energy_productive_kwh,
    COALESCE(seh.energy_non_productive_kwh, 0) AS energy_non_productive_kwh,
    COALESCE(seh.energy_non_productive_stop_kwh, 0) AS energy_non_productive_stop_kwh,

    COALESCE(csh.good_cycle_energy_kwh, 0) AS good_cycle_energy_kwh,
    ...
    CASE
        WHEN COALESCE(csh.total_parts, 0) > 0
        THEN csh.scrap_parts::numeric / csh.total_parts
        ELSE 0
    END AS scrap_rate,

    CASE
        WHEN COALESCE(csh.good_parts, 0) > 0
        THEN COALESCE(csh.good_cycle_energy_kwh, 0) / csh.good_parts
        ELSE 0
    ...
    CASE
        WHEN COALESCE(seh.energy_total_kwh, 0) > 0
        THEN (
            COALESCE(seh.energy_non_productive_kwh, 0)
            + COALESCE(seh.energy_non_productive_stop_kwh, 0)
        ) / seh.energy_total_kwh
        ELSE 0
